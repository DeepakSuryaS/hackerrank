...
class Result {
  ...
  public static List<Integer> dynamicArray(int n, List<List<Integer>> queries) {
    ...
    for(int i = 0; i < queries.size(); i++) {
      ...
      if(q.get(0) == 1) {
        // find the sequence, seq, at index ((x ^ lastAnswer) % n) in seqList and append y to seq
        seqList.get((q.get(1) ^ lastAnswer) % n).add(q.get(2));
      } else {
        // find the sequence, seq, at index ((x ^ lastAnswer) % n) in seqList
        // find the element y % size in seq (where size is the size of seq) and assign it to lastAnswer
